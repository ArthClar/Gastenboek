...
require_once "errorGastenlijst.php";
class Gast
{
    private $id;
    private $naam;
    ...
    public function emailReedsInGebruik()
    {
        ...
        $stmt = $dbh->prepare("SELECT * FROM gasten WHERE email = :email");
    ...
    public function register()
    {
        ...
        $stmt = $dbh->prepare("INSERT INTO gasten (naamGast, email, wachtwoord) VALUES
(:naamGast, :email, :wachtwoord)");
        $stmt->bindValue(":naamGast", $this->naam);
        $stmt->bindValue(":email", $this->email);
    ...
    public function login()
    {
        ...
        $stmt = $dbh->prepare("SELECT id, naamGast, wachtwoord FROM gasten WHERE email =
:email");
        $stmt->bindValue(":email", $this->email);
        $stmt->execute();
